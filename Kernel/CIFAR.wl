...
$CIFARVersion = Get[
    FileNameJoin[{ParentDirectory[$CIFARPath], "PacletInfo.wl"}]
]["Version"];
...
Get[FileNameJoin[{$CIFARPath, "Usage.wl"}]];


(*Public options*)
Options[CIFARReduce] = {CheckContractions -> True, UseCache -> True, Tabulate -> True, EliminateInvariant -> None};
...
(*ToFFLoopForm[expr_] :=
    Module[{result = 0, resultList = List @@ (Expand[expr] + zero) /.
        ...
        Get[$CIFARPath <> "lib/deltaA.wl"];
        Get[$CIFARPath <> "lib/ff.wl"];
...
AdjointReduce[expr_, fullyContractedQ_:False, opts : OptionsPattern[]
    ...
    Module[{expr0, exprsimp, StandardizeIndicesLoc, n = 0},
        ...
        Block[
            ...
            Get[$CIFARPath <> "lib/Attributes.wl"];
            Get[$CIFARPath <> "lib/ff.wl"];
            Get[$CIFARPath <> "lib/FFLoop.wl"];
            Get[$CIFARPath <> "lib/deltaA.wl"];
...
ApplyInvariantElimination[result_, None] := result;
ApplyInvariantElimination[result_, CF] := result /. CF -> TF DA/DF;
ApplyInvariantElimination[result_, DF] := result /. DF -> TF DA/CF;
ApplyInvariantElimination[result_, TF] := result /. TF -> CF DF/DA;
...
CIFARReduce[expr_, opts : OptionsPattern[]] :=
    Module[{result, lookup, fullyContractedQ = True},
        ...
        Return[ApplyInvariantElimination[result, OptionValue[EliminateInvariant]]];
    ]
